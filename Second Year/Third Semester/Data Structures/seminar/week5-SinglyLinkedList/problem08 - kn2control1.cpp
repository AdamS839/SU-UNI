...
void reevaluate(LinkedList<std::stack<T>> &list){
    ...
    // make a stack for adding sorted values
    ...
    while(it){
        if(pos % 2 == 0){ // do only for stacks on even positions
            //copy the stack
            std::stack<T> currStack = *it;
            
            // begin the cycle until copied stack is empty
            while(!currStack.empty()){
                // get the element value from top
                T curr = currStack.top();
                currStack.pop();

                // move from temp stack until we find place for the current element
    ...
    // reverse the stack for descending (already is ascending)
    std::stack<T> final;
